...
/*		var rect = canvas.getBoundingClientRect();
	...
	function checkKey(e) {
		...
		switch(e.keyCode){
			case 38:tranZ-=int;break; //up arrow
            case 87:tranZ-=int;break; //W
			case 40:tranZ+=int;break; //down arrow
            case 83:tranZ+=int;break; //S
			case 37:tranX-=int;break; //left arrow
            case 65:tranX-=int;break; //A
			case 39:tranX+=int;break; //right arrow
            case 68:tranX+=int;break; //D
			case 8 :zoom(-1)  ;break; //back space
			case 88:zoom(-1)  ;break; //Z
            case 69:zoom(-1)  ;break; //E
			case 32:zoom(1)   ;break; //space bar
			case 90:zoom(1)   ;break; //X
            case 81:zoom(1)   ;break; //Q
			default:break;
		}
		translate();
	}
	...
	function cluster(){
		...
		for(var i=0;i<pointsArray.length;i++){
			...
			if(pointsArray[i][3]>maxZ){maxZ=pointsArray[i][3];}
			if(pointsArray[i][3]<minZ){minZ=pointsArray[i][3];}
		}
    ...
    function redraw(){
        ...
        function drawGrid(){
            ...
            for(var i=1;i<=span/light;i=i+1){
                ...
                if(i%dark===0){m.lineWidth=2;m.strokeStyle='#080808';}
                m.beginPath();
                m.moveTo(-len,dis);
                m.lineTo(len,dis);
                m.moveTo(-len,-dis);
			...
			//draw origin
            m.strokeStyle='#000';m.lineWidth = 3;
            m.beginPath();
            m.moveTo(0,-len);
            m.lineTo(0,len);
            m.moveTo(-len,0);
...
//DEBUG/////draw static canvas center
            ...
            for(var i=0;i<foundersArray.length;i++){
                ...
                m.fillText(founder,22,pos+5);m.fill();
                pos += 20;
            }
